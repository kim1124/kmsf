...
Playground의 `Td Cell 예제`는 `cellSelection` 토글 대신 포맷, 스타일, renderer, clipboard guard, cell event callback 중심으로 구성한다.
...
- `셀` 예제는 format, column props style/className/disabled, renderer, cell click, double click, context menu, keyboard event를 각각 상태 출력으로 보여준다.
