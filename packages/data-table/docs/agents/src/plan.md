...
- 2026-06-30: Theme customization implementation follows `docs/agents/src/2026-06-30-theme-customization-plan.md`. `ask-question` decisions are closed: keep sample themes on `theme.className`, do not add `theme.preset`, do not include row height presets in Theme, keep `rowHeight` as the virtualized height source of truth, and expand package `styles.css` to table shell plus theme classes.
- 2026-06-30: ReactDataTable-style docs shell + live playground improvement follows `docs/superpowers/plans/2026-06-30-reactdatatable-style-playground/plan.md`. `ask-question` / `ask-plan` decisions are closed: use React Router routes, remove the current top-level tab split in favor of routed article pages, keep Korean UI/copy, exclude search from the first pass, expose only implemented features in the sidebar, and use `prism-react-renderer` for example code styling.
- 2026-06-28: 2 depth header / column group implementation follows `docs/agents/src/2026-06-28-2-depth-header-implementation-plan.md`. `ask-research` / `ask-question` / `ask-plan` decisions are closed: use flat `columns` plus separate `columnGroups`, use `columnGroups[].label`, support parent resize with ratio-preserving child widths and min/max redistribution, support parent move as a child block, keep child group-out movement unsupported, use group hidden state for actual column visibility, keep header-wide hide/show independent, render ungrouped columns with `rowSpan=2`, and limit support to max 2 depth.
