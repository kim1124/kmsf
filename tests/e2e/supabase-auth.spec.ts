...
test("initial admin setup, auth flow, page checks, and member deletion", async ({ page }) => {
  ...
  const kimEmail = `sampleuser_${runId}@mailinator.com`;
  ...
  await page.waitForLoadState("networkidle");
  await page.locator("#sign-up-username").fill("sampleuser");
  ...
  await expect(page.getByRole("heading", { name: "대시보드" })).toBeVisible();
  await expectMainPages(page, "sampleuser");

  await page.getByRole("button", { name: "프로필 메뉴" }).click();
  await expect(page.getByText("sampleuser").first()).toBeVisible();
  ...
  await page.locator("#login-username").fill("sampleuser");
  await page.locator("#login-password").fill("member00@!");
